...
/**
 * ComicSendToKindleButton Component - Uses STK (Send to Kindle) API for comics
 *
 * @param {Object} props
 * @param {number} props.comicId - Comic ID
 * @param {number} props.issueId - Issue ID to send
 * @param {string|null} props.sentAt - Datetime when it was sent (null if never sent)
 * @param {boolean} props.hasFile - Whether the file exists
...
const ComicSendToKindleButton = ({
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // Send via comic API
      const response = await comicApi.sendToKindle(comicId, issueId);

      if (response.data.success) {
        setStatus('success');
        if (onSent) {
          onSent(issueId, new Date().toISOString());
        }
...
export default ComicSendToKindleButton;
